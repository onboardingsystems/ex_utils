defmodule Obs.Plug do
  ...
  defmacro __using__(opts) do
    quote do
      ...
      def call(state, opts \\ []) do
        plug_builder_call(state, opts)
      end
  ...
  defmacro __before_compile__(env) do
    ...
    {state, body} = __MODULE__.compile(env, plugs, builder_opts)

    quote do
      defp plug_builder_call(unquote(state), _), do: unquote(body)
    end
  end
  ...
  def compile(env, pipeline, builder_opts) do
    ...
    {state, Enum.reduce(pipeline, state, &quote_plug(init_plug(&1), &2, env, builder_opts))}
  end
  ...
  defp quote_plug_call(:function, plug, opts) do
    quote do: unquote(plug)(state, unquote(Macro.escape(opts)))
  end

  defp quote_plug_call(:module, plug, opts) do
    quote do: unquote(plug).call(state, unquote(Macro.escape(opts)))
  end
